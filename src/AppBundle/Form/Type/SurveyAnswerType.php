...
class SurveyAnswerType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) use (&$surveyQuestion) {
            $surveyQuestion = $event->getData();
            dump($surveyQuestion);
            //dump($surveyQuestion->getA());
            $form = $event->getForm();
            ...
            switch($surveyQuestion->getType()) {
                case "list": // This creates a dropdown list if the type is list
                    $choices = $this->createChoices($surveyQuestion);

                    $form->add('question', 'choice', array(
                        'label' => $surveyQuestion->getQuestion(),
                        'help' => $surveyQuestion->getHelp(),
                        'choices' => $choices
                    ));
                ...
                default: // This creates a textarea if the type is text (default)
                    $type = "text";
                    $form->add('question', 'textarea', array(
                        'label' => $surveyQuestion->getQuestion(),
                        'help' => $surveyQuestion->getHelp()
                    ));
            }
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, function(FormEvent $event) use (&$surveyQuestion) {
            $form = $event->getForm();
            ...
            switch($surveyQuestion->getType()) {
                case "list":
                    ...
                    $choices[$data['question']] = $data['question'];
                ...
                case "check":
                    ...
                    $questions = array_combine($data['question'],$data['question']);
                    $newChoices = array_merge($choices, $questions);
     ...
     * @return array
     */
    public function createChoices($surveyQuestion) {
        $alternatives = $surveyQuestion->getAlternatives();
