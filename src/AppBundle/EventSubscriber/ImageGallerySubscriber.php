...
class ImageGallerySubscriber implements EventSubscriberInterface
{
    ...
    public function logCreatedEvent(ImageGalleryEvent $event)
    {
        ...
        $this->logger->info('New image gallery *'. $imageGallery->getTitle() . '* on endpoint /' . $imageGallery->getReferenceName(). '.');
    }

    public function addCreatedFlashMessage(ImageGalleryEvent $event)
    {
        $imageGallery = $event->getImageGallery();
        $this->session->getFlashBag()->add('success', 'Bildegalleriet er opprettet på endpointet /' . $imageGallery->getReferenceName() . '.');
    }

    public function logDeletedEvent(ImageGalleryEvent $event)
    {
        $imageGallery = $event->getImageGallery();
        $this->logger->info('Image gallery *'. $imageGallery->getTitle() . '* on endpoint /' . $imageGallery->getReferenceName(). ' has been deleted.');
    }

    public function addDeletedFlashMessage(ImageGalleryEvent $event)
    {
        $imageGallery = $event->getImageGallery();
        $this->session->getFlashBag()->add('success', 'Bildegalleriet på endpointet /' . $imageGallery->getReferenceName() . ' har blitt slettet.');
    }

    public function logEditedEvent(ImageGalleryEvent $event)
    {
        $imageGallery = $event->getImageGallery();
        $this->logger->info('Image gallery *'. $imageGallery->getTitle() . '* on endpoint /' . $imageGallery->getReferenceName(). ' was edited.');
    }
